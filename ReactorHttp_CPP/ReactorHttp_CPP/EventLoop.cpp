...
EventLoop::EventLoop(const int dispatcherType) :EventLoop(string(), dispatcherType)
{
}

EventLoop::EventLoop(const std::string threadName, const int dispatcherType)
{
	m_isQuit = true;
	m_threadID = this_thread::get_id();
	m_threadName = threadName == string() ? "MainThread" : threadName;
	m_dispatcher = getDispatcher(this, dispatcherType);
